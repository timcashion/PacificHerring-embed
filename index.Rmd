...
cpi <- read.csv("./Data/Canadian_CPI.csv") %>% 
...
<div class="dropdown" id="dropdown">
  ...
  <div class="dropdown-content">
    ...
    <a href="#the-fisheries">The Fisheries</a>
    <a href="#the-products">The Products</a>
    <a href="#the-supply-chain">The Supply Chain</a>
    <a href="#value-of-the-fisheries">Value of the Fisheries</a>
    <a href="#ownership-and-licenses">Ownership and Licenses</a>
    <a href="#how-does-the-roe-herring-fishery-stack-up">How does the roe herring fishery stack up?</a>
    <a href="#going-forward-a-look-into-closing-the-roe-herring-fishery">Going forward</a>
...
At the recent peak of the herring roe fishery, in 1989, coast-wide landings exceeded 40 thousand tonnes, worth \$128 million for fishers (or \$3.17 per kg). At this time, there were 427 full time-equivalent jobs in the fishery. Following a recent low in 2011, coast-wide landings increased to approximately 20 thousand tonnes in 2017. This catch was worth \$17.2 million to fishers (or \$0.84 per kg), and the fishery generated 216 full time-equivalent jobs.
...
season_summary_total <- season_summary %>% 
  group_by(Year, Area) %>% 
  summarize(Value=sum(Value)) %>% 
  ungroup() %>% 
...
season_summary_plot <- season_summary %>% 
  bind_rows(season_summary_total) %>% 
...
herring_lic_area_plot <- net_change_area_licences %>% 
  ...
  theme(axis.title.y = element_text(size = 3, margin = margin(t = 0, r = 10, b = 0, l = 0))) +
  #theme(plot.title = element_text(hjust = 0.5)) + 
  NULL 
ggsave("./Images/Net licence movement for the two main roe fishing areas.tiff", dpi=300)

herring_lic_area_plot <- herring_lic_area_plot +
  ylab("")
ggplotly(herring_lic_area_plot) %>% 
  #layout(font=plot_font)  %>% 
  layout(yaxis = list(title = paste0(c("Number of herring licences   \nswitching to new area", rep("\n", 2)), collapse = "")))
...
pattison_roe_combined <- top_10_company_table$n[which(top_10_company_table$Licence_Holder=="JIM PATTISON GROUP")]/ sum(top_10_company_table$n) #Pattison companies own 15.5% of licences. 
...
licence_value_plot <- licence_value_plot +
  ylab("")
ggplotly(licence_value_plot) %>% 
  #layout(font=plot_font)  %>% 
  layout(yaxis = list(title = paste0(c("Value (1000 $)\n", rep("\n", 2)), collapse = "")))
